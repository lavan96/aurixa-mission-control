  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const status =
          res?.error === "device_limit_reached" ? 402 : res?.error === "seat_not_found" ? 404 : 200;
        if (res?.ok && !res?.idempotent) {
          fireSeatWebhook(
            "devices.registered" as never,
            { external_user_id: d.external_user_id, ...res },
            key.clone_id,
          ).catch(() => {});
        }
        if (res?.error === "device_limit_reached") {
          fireSeatWebhook(
            "devices.limit.reached" as never,
            { external_user_id: d.external_user_id, ...res },
            key.clone_id,
          ).catch(() => {});
        }
