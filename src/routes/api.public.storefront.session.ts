...
/**
 * GET /api/public/storefront/session?session_id=cs_…&h=<uuid>
 *
 * Receipt data for the Aurixa Systems website's post-checkout pages.
 * Authorised by possession of the (session_id, handoff_id) pair — it only
 * ever exists in the redirect URL Stripe sent that purchaser to. Returns the
 * purchase summary, live fulfilment status (poll until `fulfilled`), and the
 * validated return link back to the purchaser's command center.
 */
  ...
  server: {
    handlers: {
      ...
      GET: async ({ request }) => {
        ...
        if (!sessionId || sessionId.length > 200 || !h) {
          return storefrontJson({ ok: false, error: "invalid_input" }, 400);
        }

        try {
          if (!(await sessionMatchesHandoff(sessionId, h))) {
